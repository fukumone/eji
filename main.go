...
func main() {
  var reverseEmojiCodeMap = map[string]string{}
  for key, value := range emojiCodeMap {
    reverseEmojiCodeMap[value] = key
  }
  ...
  for _, key := range emojiNameArray {
    value := emojiCodeMap[key]
    emojiValues += value + "  "
    command := cli.Command{
      Name: key,
      Usage: "output " + value,
  ...
  for _, value := range emojiValueArray {
    emoji := fmt.Sprintf("%s", value)
    val := reverseEmojiCodeMap[emoji]
    command := cli.Command{
      Name: emoji,
      Usage: "output " + val,
      Action: func(c *cli.Context) error {
	...
	command2 := cli.Command{
		Name: "index",
    Aliases: []string{"i"},
    Usage: "Display a list of emoji",
		Action: func(c *cli.Context) error {
			fmt.Println(emojiValues)
			return nil
		},
	}
